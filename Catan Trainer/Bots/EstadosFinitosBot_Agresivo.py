...
# Este bot sigue una estrategia de control sobre las rutas del tablero, construyendo siempre las carreteras hacia nodos donde confrontar a enemigos
# Además, trata de conseguir la carretera más larga y el ejército más grande

class EstadosFinitosBot_Agresivo(BotInterface):
    """
                    ...
                    for road_obj in possibilities:
                        for node in  self.board.__get_adjacent_nodes__(road_obj['finishing_node']):
                            if self.board.nodes[node]['player'] != -1 and self.board.nodes[node]['player'] != self.id:
                                possible_roads_torwards_oponent.append(road_obj)
                    if len(possible_roads_torwards_oponent):
                            road_node = random.randint(0, len(possible_roads_torwards_oponent) - 1)
                            return {'building': BuildConstants.ROAD,
                                    'node_id': possible_roads_torwards_oponent[road_node]['starting_node'],
                                    'road_to': possible_roads_torwards_oponent[road_node]['finishing_node']}
