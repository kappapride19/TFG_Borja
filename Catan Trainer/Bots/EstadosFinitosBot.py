...
class EstadosFinitosBot(BotInterface):
    """
    ...
    year_of_plenty_material_one = MaterialConstants.CLAY
    year_of_plenty_material_two = MaterialConstants.WOOD
    estado_actual = EstadosConstants.CIUDAD
    ...
    def on_game_start(self, board_instance):
        ...
        for node_id in possibilities:
                ...
                if len(set(terrains_type)) >= max_diff and self.common_data(list(set(missing)),list(set(terrains_type))) and 2 in terrains_type:
        ...
        for node_id in adjacent_nodes:
            ...
            for node_id_lvl2 in adjacent_nodes_lvl_2:
                if self.board.nodes[node_id_lvl2]['player'] != -1 and self.board.nodes[node_id_lvl2]['player'] != self.id:
                    possible_roads_torwards_oponent.append(node_id)

        if len(possible_roads_torwards_oponent) > 0 :
            chosen_road_to_id = possible_roads_torwards_oponent[random.randint(0, len(possible_roads_torwards_oponent) - 1)]
        else:
    ...
    def change_state(self):
        ...
        match self.estado_actual:
            case EstadosConstants.CARRETERA:
                if self.hand.resources.has_this_more_materials(BuildConstants.CITY) or self.town_number == 0 and missing_terrain == 0:
                    estado_nuevo = EstadosConstants.CIUDAD
                else:
                    if (self.hand.resources.has_this_more_materials(BuildConstants.TOWN) and len(possibilities) !=0) or (len(place_to_build_town) !=0 and missing_terrain != 0):
                        estado_nuevo = EstadosConstants.POBLADO
                    else:
                        if not self.hand.resources.has_this_more_materials(BuildConstants.ROAD) or len(place_to_build_town) !=0 :
                            estado_nuevo = EstadosConstants.CARTAS
            case EstadosConstants.CIUDAD:
                if not self.hand.resources.has_this_more_materials(BuildConstants.CITY) or self.town_number == 0 or len(missing_terrain) != 0:
                    if (self.hand.resources.has_this_more_materials(BuildConstants.TOWN) and len(possibilities) !=0) or (len(place_to_build_town) !=0 and missing_terrain != 0):
                        estado_nuevo = EstadosConstants.POBLADO
                    else:
                        if self.hand.resources.has_this_more_materials(BuildConstants.ROAD) and len(place_to_build_town) ==0 and number_of_roads < 15:
                            estado_nuevo = EstadosConstants.CARRETERA
                        else :
                            estado_nuevo = EstadosConstants.CARTAS
            case EstadosConstants.POBLADO:
                if self.hand.resources.has_this_more_materials(BuildConstants.CITY) and self.town_number != 0 and missing_terrain == 0:
                    estado_nuevo = EstadosConstants.CIUDAD
                else:
                    if (not self.hand.resources.has_this_more_materials(BuildConstants.TOWN) or len(possibilities) == 0 or (len(place_to_build_town) !=0 and missing_terrain != 0)) :
                        if self.hand.resources.has_this_more_materials(BuildConstants.ROAD) and len(place_to_build_town) ==0 and number_of_roads < 15:
                            estado_nuevo = EstadosConstants.CARRETERA
                        else :
                            estado_nuevo = EstadosConstants.CARTAS
            case EstadosConstants.CARTAS:
                if self.hand.resources.has_this_more_materials(BuildConstants.CITY) and self.town_number != 0 and len(missing_terrain) == 0 and missing_terrain == 0:
                    estado_nuevo = EstadosConstants.CIUDAD
                else:
                    if (self.hand.resources.has_this_more_materials(BuildConstants.TOWN) and len(possibilities) !=0) or (len(place_to_build_town) !=0 and missing_terrain != 0):
                        estado_nuevo = EstadosConstants.POBLADO
                    else:
                        if not self.hand.resources.has_this_more_materials(BuildConstants.ROAD) or len(place_to_build_town) !=0 or number_of_roads >= 15:
                            estado_nuevo = None
                        else : estado_nuevo = EstadosConstants.CARRETERA
            case _:
                estado_nuevo = EstadosConstants.CIUDAD
