...
class EstadosFinitosBot_Poblados(BotInterface):
    """
    ...
    def on_game_start(self, board_instance):
        ...
        for node_id in possibilities:
                if len(list(set(self.board.nodes[node_id]['contacting_terrain']))) >= max_diff:
                    if per_prob.__contains__(node_id):
                        per_diff.append(node_id)
                        max_diff = len(list(set(self.board.nodes[node_id]['contacting_terrain'])))
    ...
    def change_state(self):
        ...
        match self.estado_actual:
            case EstadosConstants.CARRETERA:
                if self.hand.resources.has_this_more_materials(BuildConstants.CITY) or self.town_number == 0:
                    estado_nuevo = EstadosConstants.CIUDAD
                else:
                    if self.hand.resources.has_this_more_materials(BuildConstants.TOWN) and len(possibilities) !=0:
                        estado_nuevo = EstadosConstants.POBLADO
                    else:
                        if not self.hand.resources.has_this_more_materials(BuildConstants.ROAD):
                            estado_nuevo = EstadosConstants.CARTAS
            case EstadosConstants.CIUDAD:
                if not self.hand.resources.has_this_more_materials(BuildConstants.CITY) or self.town_number == 0:
                    if self.hand.resources.has_this_more_materials(BuildConstants.TOWN) and len(possibilities) !=0:
                        estado_nuevo = EstadosConstants.POBLADO
                    else:
                        if self.hand.resources.has_this_more_materials(BuildConstants.ROAD):
            ...
            case EstadosConstants.POBLADO:
                if self.hand.resources.has_this_more_materials(BuildConstants.CITY) or self.town_number == 0:
                    ...
                    if not self.hand.resources.has_this_more_materials(BuildConstants.TOWN) or len(possibilities) ==0:
                        if self.hand.resources.has_this_more_materials(BuildConstants.ROAD):
                            estado_nuevo = EstadosConstants.CARRETERA
                        else :
                            estado_nuevo = EstadosConstants.CARTAS
